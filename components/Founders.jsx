    ...
    <section id="founders" style={{ padding: '140px 0', borderTop: '1px solid rgba(0,0,0,.06)' }}>
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
          {people.map((p) => (
            <div key={p.name} style={{ padding: '32px 28px', border: '1px solid rgba(0,0,0,.1)', borderRadius: 24, display: 'flex', flexDirection: 'column', gap: 20, minHeight: 280, background: '#fff' }}>
              <div style={{
                width: 72, height: 72, borderRadius: '50%',
                background: 'rgba(0,0,0,.04)', border: '1px solid rgba(0,0,0,.1)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontFamily: 'Tektur, sans-serif', fontSize: 22, color: 'rgba(0,0,0,.6)',
              }}>{p.name.split(' ').map(s => s[0]).join('')}</div>
              <div>
                <h3 style={{ fontFamily: 'Tektur, sans-serif', fontSize: 22, lineHeight: 1.15, letterSpacing: '-0.01em', margin: '0 0 6px', fontWeight: 400, color: '#000' }}>{p.name}</h3>
              ...
              {p.link && (
                <a href={p.link} target="_blank" rel="noopener" style={{ fontSize: 13, color: '#000', borderBottom: '1px solid rgba(0,0,0,.2)', paddingBottom: 2, alignSelf: 'flex-start' }}>
                  Подробнее →
                </a>
              )}
            </div>
